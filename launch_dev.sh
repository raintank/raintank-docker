...
screen -S raintank -p nsq_metrics_tank -X stuff '/wait.sh cassandra:9042 && ./nsq_metrics_tank --chunkspan 120 --numchunks 5 --statsd-addr statsdaemon:8125 --nsqd-tcp-address nsqd:4150 --listen :6063 --cassandra-addrs cassandra &> /var/log/raintank/nsq_metrics_tank.log &\n'
screen -S raintank -p nsq_metrics_tank -X stuff 'tail -f /var/log/raintank/nsq_metrics_tank.log\n'
...
screen -S raintank -p nsq_metrics_to_elasticsearch -X stuff './nsq_metrics_to_elasticsearch --elastic-addr elasticsearch:9200 --redis-addr redis:6379 --statsd-addr statsdaemon:8125 --nsqd-tcp-address nsqd:4150 &> /var/log/raintank/nsq_metrics_to_elasticsearch.log &\n'
screen -S raintank -p nsq_metrics_to_elasticsearch -X stuff 'tail -f /var/log/raintank/nsq_metrics_to_elasticsearch.log\n'
...
screen -S raintank -p nsq_probe_events_to_elasticsearch -X stuff './nsq_probe_events_to_elasticsearch --elastic-addr elasticsearch:9200 --statsd-addr statsdaemon:8125 --nsqd-tcp-address nsqd:4150 &> /var/log/raintank/nsq_probe_events_to_elasticsearch.log &\n'
screen -S raintank -p nsq_probe_events_to_elasticsearch -X stuff 'tail -f /var/log/raintank/nsq_probe_events_to_elasticsearch.log\n'
