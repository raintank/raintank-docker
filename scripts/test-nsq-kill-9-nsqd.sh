...
while ! docker exec rt_grafana_1 grep -q PUBLISH /var/log/raintank/grafana-dev.log; do
  sleep 1
done
...
pid=$(docker exec rt_nsqd_1 ps aux | grep /nsqd | grep -v while | awk '{print $1}')
docker exec rt_nsqd_1 kill -9 $pid
log "wait a minute..."
sleep 60
